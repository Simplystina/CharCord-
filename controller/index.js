  ...
  if (parseInt(msg) == 99) {
     ...
     if(!order.ordermade){
      ...
      resp = `
             ...
             `
     resp && io.to(room).emit("message", formatMessage(botName, resp));
     console.log(deviceID,"deviceId Iddddddddddd")
  ...
  if(parseInt(msg) == 1){
   ...
   order.ordermade = "food"
    resp = getMenu(menu.food)
     console.log(order.ordermade, "order made heer")
...
exports.fullOrderLogic = async(io)=>{
  ...
  socket.on('deviceId', deviceId => {
    ...
    let username
    socket.on('name', (name) =>{
      console.log(name,"nameeeeeeeeeeeeeee")
      username= name
    })
